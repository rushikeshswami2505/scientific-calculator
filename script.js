...
onebyxElement.addEventListener('click', function () {
    ...
    while(--len>=0)
    {
        if(isDigit(str.charAt(len)) || str.charAt(len)==='.')
        {
            n = parseInt(str.charAt(len)) + n;
        }
        else break;
    }
 
    console.log(n);
    str = str.substring(0,len+1) + '1/('+n;
    equation.innerText = equation.innerText.substring(0,len+1) + '1/('+n;
    console.log(str);
});
